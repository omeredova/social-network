...
export function ProfileActivityTabs({ profile }: ProfileActivityTabsProps) {
  return (
    <Tabs defaultSelectedKey="posts" className="mt-6" aria-label="Profile activity">
      <TabsList variant="line" className="w-full justify-start border-b border-post-border p-0">
        <TabsTrigger
          id="posts"
      ...
      <TabsContent id="posts" className="mx-auto mt-6 max-w-2xl">
        <section aria-label="User posts" className="space-y-5">
          {profile.posts.map((post) => <PostCard key={post.id} post={post} linked />)}
        </section>
      </TabsContent>
        ...
        <section aria-label="User comments">
          <Card className="rounded-profile-card border-post-border bg-post-surface px-5 shadow-post-card">
            {profile.comments.length > 0 ? (
              profile.comments.map((comment) => <CommentCard key={comment.id} comment={comment} />)
            ) : (
